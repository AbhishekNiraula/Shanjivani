...
to_whatsapp = 'whatsapp:+9779807941286'  # Your Twilio WhatsApp number
    ...
    try:
        if msg.topic == ULTRASONIC_TOPIC:
            ...
            if distance_data < 10:
                ...
                socketio.emit('emergency_signal', {'message': 'Emergency Signal Triggered!', 'distance': distance_data})
            else:
        ...
        if msg.topic == BUTTON_EMERGENCY:
            ...
            socketio.emit('button_emergency', {'message': 'Button Emergency Triggered!'})
        else:
            button_emergency = False
...
def trigger_whatsapp_message():
    message = twilio_client.messages.create(
		body='Hello this is from Sanjivani, Emergency detected! Please take action immediately.',
		from_=from_whatsapp,
		to=to_whatsapp
	)
...
def emergency():
    ...
    if button_emergency:
        ...
        trigger_type = "Emergency Triggered"
        data = "Dispatch Service As Soon As Possible!"
